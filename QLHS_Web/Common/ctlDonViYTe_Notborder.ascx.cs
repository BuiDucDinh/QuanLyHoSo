...
public partial class Common_ctlDonViYTe_Notborder : System.Web.UI.UserControl
{
        ...
        if (!this.IsPostBack)
        {
            ...
            switch (Session["G_Ma_Loai_Hinh"].ToString())
            {
                case "1":
                    ...
                    {
                        DataTable myData = Sys_Common.RunTableBySQL("SELECT * FROM HT_Don_Vi_YT WHERE Loai_Hinh IN (4,5,7) AND TB=1 or (Loai_Hinh = 6 and Thanh_Phan = 1) ");
                        if (myData != null)
                        {
