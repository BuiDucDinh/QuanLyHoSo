...
public partial class Common_cltDonViYTe_TuyenDT : System.Web.UI.UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!this.IsPostBack)
        {
            cboLoaiHinhChon.Value = Sys_Common.G_LOAI_HINH_KHAM_CHUA_BENH;
            switch (Session["G_Ma_Loai_Hinh"].ToString())
            {
                case "1":
                    ...
                    {
                        DataTable myData = Sys_Common.RunTableBySQL("SELECT * FROM HT_Don_Vi_YT WHERE Loai_Hinh IN (4,5) or (Loai_Hinh = 6 and Thanh_Phan = 1) or (Loai_Dac_Biet=2) ");
                        if (myData != null)
                        {
            ...
            cboLoaiHinhChon.Value = "0";
            cboLoaiHinhChon.ReadOnly = true;
    ...
    protected void cboLoaiHinhChon_Selected(object sender, DirectEventArgs e)
    {
        if (cboLoaiHinhChon.Value.ToString() == Sys_Common.G_LOAI_HINH_KHAM_CHUA_BENH.ToString())
        {
            DataTable myData = Sys_Common.RunTableBySQL("SELECT * FROM HT_Don_Vi_YT WHERE Loai_Hinh IN (4,5,7) or (Loai_Hinh = 6 and Thanh_Phan = 1) ");
            if (myData != null)
            {
                dsDonViKhacChon.DataSource = myData;
                dsDonViKhacChon.DataBind();
