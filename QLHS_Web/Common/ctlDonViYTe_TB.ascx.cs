...
using QLHS_Logic;
using System.Data;
public partial class Common_ctlDonViYTe_TB : System.Web.UI.UserControl
{
        ...
        if (!this.IsPostBack)
        {
            ...
            switch (Session["G_Ma_Loai_Hinh"].ToString())
            {
                case "1":
                    ...
                    {
                        DataTable myData = Sys_Common.RunTableBySQL("SELECT * FROM HT_Don_Vi_YT WHERE Loai_Hinh IN (4,5) AND TB=1 or (Loai_Hinh = 6 and Thanh_Phan = 1) ");
                        if (myData != null)
                        {
                    ...
                    else if (cboLoaiHinhChon.Value.ToString() == Sys_Common.G_LOAI_HINH_PHONG_BAN_THUOC_SO_YTE.ToString())
                    {
                        DataTable myData = Sys_Common.RunTableBySQL("SELECT * FROM HT_Don_Vi_YT WHERE Loai_Hinh IN (7) AND TB=1 ");
                        if (myData != null)
                        {
                            dsDonViKhacChon.DataSource = myData;
                            dsDonViKhacChon.DataBind();
    ...
    protected void cboLoaiHinhChon_Changed(object sender, DirectEventArgs e)
    {
    ...
    {
        DataTable myTable = Sys_Common.RunTableBySQL("SELECT Ma_Don_Vi, Ten_Don_Vi FROM HT_Don_Vi_YT WHERE Ma_Huyen='" + cboMaHuyenChon.Value.ToString() + "' AND (Loai_Hinh=3 or Loai_Hinh=2)");
        if (myTable != null)
        {
