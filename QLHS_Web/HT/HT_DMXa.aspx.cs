...
public partial class HT_HT_DMXa : System.Web.UI.Page
{
    ...
    {
        //Load combo data
        dsDM_Huyen.DataSource = Sys_Common.HT_DM_HUYEN.Danh_Sach();
        dsDM_Huyen.DataBind();
    }
    ...
    protected void dsData_RefreshData(object sender, StoreRefreshDataEventArgs e)
    {
        ...
        try
        {
            if (cboMa_Huyen.Value.ToString() != "")
            {
                where = " and Ma_Huyen='" + cboMa_Huyen.Value.ToString() + "'";
            }
        }
        catch { }
        if (e.Parameters["Filter"] == "")
        {
            this.odsData.SelectParameters["WhereString"].DefaultValue = "(select * from HT_DM_Xa where 1=1" + where + ") as A";
        }
        else
        {
            this.odsData.SelectParameters["WhereString"].DefaultValue = "HT_DM_Xa WHERE "
                + " OR Ten_Xa like N'%" + txtFilter.Text + "%'"
                + " OR Ma_Xa ='" + txtFilter.Text + "'";
    ...
    protected void CboMa_Huyen_Change(object sender, DirectEventArgs e)
    {
    ...
    protected void grid_DbClick(object sender, DirectEventArgs e)
    {
        ...
        foreach (SelectedRow row in sm.SelectedRows)
        {
            ...
            this.SetData(Sys_Common.HT_DM_XA.Lay(id));
            break;
        }
    ...
    protected void btnDelete_Click(object sender, DirectEventArgs e)
    {
        ...
        foreach (SelectedRow row in sm.SelectedRows)
        {
            Sys_Common.HT_DM_XA.Xoa(row.RecordID.ToString());
        }
        sm.SelectedRows.Clear();
        sm.UpdateSelection();
    ...
    private void SetData(HT_DM_Xa_Chi_Tiet myDetail)
    {
        btnUpdate.Text = "Cập nhật";
        btnUpdate.Icon = Icon.ApplicationEdit;

        this.txtTen_Xa.Text = myDetail.Ten_Xa;
        this.txtSo_Thu_Tu.Text = myDetail.STT.ToString();
        this.cboMa_Huyen.Value = myDetail.Ma_Huyen;


        //hidden attributes
        this.txtMa_Xa.Text = myDetail.Ma_Xa.ToString();
    }
    ...
    private void ClearData()
    {
        ...
        this.txtTen_Xa.Text = "Xã ";

        this.txtSo_Thu_Tu.Text = this.txtSo_Thu_Tu.Text != "" ? (int.Parse(this.txtSo_Thu_Tu.Text) + 1).ToString() : "";
        this.cboHinh_Nen.Value = "";
        this.cboMa_Huyen.Value = "";
    ...
    protected void btnUpdate_Click(object sender, DirectEventArgs e)
    {
        if (FormValidate())
        {
            ...
            Sys_Common.HT_DM_XA.Cap_Nhat_Them(txtMa_Xa.Text, cboMa_Huyen.Value.ToString(), txtTen_Xa.Text, txtSo_Thu_Tu.Text);
    ...
    protected void btnExport_Click(object sender, DirectEventArgs e)
    {
        ...
        List<HT_DM_Xa_Chi_Tiet> lstData = JSON.Deserialize<List<HT_DM_Xa_Chi_Tiet>>(e.ExtraParams["data"]);
        #endregion

        Type myDetail = typeof(HT_DM_Xa_Chi_Tiet);
        ...
        {
            HT_DM_Xa_Chi_Tiet appDetailItem = lstData[i];
            colIndex = 0;
            foreach (PropertyInfo info in myDetail.GetProperties())
            {
        ...
        string data = ExcelHelper.CreateExcelContentXmlVersion(range, Server.MapPath("~/Template/HT_DM_Xa.xml"));
