...
using QLHS_Logic.NV;
public partial class NghiepVu_LichSu_LichSuTongHop : System.Web.UI.Page
{
    ...
    private void Initialization()
    {
        ...
        stNguoidung.DataSource = Sys_Common.HT_NGUOI_DUNG.Danh_Sach();
        DataBind();
    }
    ...
    protected void stData_RefreshData(object sender, StoreRefreshDataEventArgs e)
    {
        ...
        if (cmbNguoidung.Value != null && !string.IsNullOrEmpty(cmbNguoidung.Value.ToString()))
        {
            dk += " and MaND=" + cmbNguoidung.Value.ToString();
        }
        ...
        this.odsData.SelectParameters["WhereString"].DefaultValue = @"(select ID,NgayThaoTac,(select Ten_Chuc_Nang from HT_Chuc_Nang dn where dn.Ma_Chuc_Nang=lg.MaForm) as ChucNang,
	                                                                    (select Ten_Dang_Nhap from HT_Nguoi_Dung nd where nd.Ma_Nguoi_Dung=lg.MaND) as TenND,
                                                                        (select (case ThaoTac 
