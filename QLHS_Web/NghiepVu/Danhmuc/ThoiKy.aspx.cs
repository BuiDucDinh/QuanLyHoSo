...
public partial class NghiepVu_Danhmuc_ThoiKy : System.Web.UI.Page
{
    ...
    protected void stData_RefreshData(object sender, StoreRefreshDataEventArgs e)
    {
        ...
        this.odsData.SelectParameters["WhereString"].DefaultValue = @"(select * from ThoiKy) as A";
    ...
    protected void gvData_DbClick(object sender, DirectEventArgs e)
    {
        ...
        if (role.Duoc_Sua)
        {
            ...
            foreach (SelectedRow row in sm.SelectedRows)
            {
                ...
                NV_ThoiKy_ChiTiet myDetail = Sys_Common.NV_ThoiKy.GetById(id);  //Sys_Common.NL_DM_BIEN_CHE.Lay(int.Parse(row.RecordID));
                SetData(myDetail);
                break;
            }
    ...
    private void SetData(NV_ThoiKy_ChiTiet model)
    {
        btnUpdate.Text = "Cập nhật";
        btnUpdate.Icon = Icon.ApplicationEdit;

        hdID.Text = model.ID.ToString();
        txtTen.Text = model.TenThoiKy;
        txtEngName.Text = model.EngName;
    ...
    protected void btnDelete_Click(object sender, DirectEventArgs e)
    {
        ...
        if (role.Duoc_Xoa)
        {
            ...
            foreach (SelectedRow row in sm.SelectedRows)
            {
                Sys_Common.NV_ThoiKy.Xoa(int.Parse(row.RecordID));
            }
            sm.SelectedRows.Clear();
            sm.UpdateSelection();
    ...
    protected void btnUpdate_Click(object sender, DirectEventArgs e)
    {
        if (FormValidate())
        {
            ...
            model.TenThoiKy = txtTen.Text;
            model.EngName = txtEngName.Text;
            model.MoTa = txtMoTa.Text;
            if (hdID.Text != "")
            {
                model.ID = int.Parse(hdID.Text);
                Sys_Common.NV_ThoiKy.update(model);
            }
            else
            {
                Sys_Common.NV_ThoiKy.them(model);
            }
            ClearData();
